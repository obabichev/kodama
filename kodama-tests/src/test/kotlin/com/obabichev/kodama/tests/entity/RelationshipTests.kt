 ...
 *
 * Features Tested:
 * - Relationship methods with context receivers (user.orders() with EntitySession in context)
        ...
        withConnection {
            EntitySession(this.connection).use { session ->
                with(session) {
                    ...
                    val orders1 = user.orders()

                    // Load orders second time
                    val orders2 = user.orders()
        ...
        withConnection {
            EntitySession(this.connection).use { session ->
                with(session) {
                    ...
                    val aliceOrders = alice.orders()
                    val bobOrders = bob.orders()
        ...
        withConnection {
            EntitySession(this.connection).use { session ->
                with(session) {
                    ...
                    // Initially no orders
                    assertEquals(0, user.orders().size, "User should have no orders initially")
