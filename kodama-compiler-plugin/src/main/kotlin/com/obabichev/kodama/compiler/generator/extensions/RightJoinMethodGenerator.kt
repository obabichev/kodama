...
/**
 * Generates the .rightJoin() extension method for adding tables with RIGHT OUTER JOIN.
 *
 * Similar to JoinMethodGenerator but creates RIGHT OUTER JOIN clauses, which include
 * all rows from the right table even if there's no match in the left table.
 *
 * Example output for Person + Order combination:
 * ```
 * inline fun <PersonSel, AC : AggCount>
 * AfterFromQueryBuilder_Person<PersonSel, AC>.rightJoin(
 *     table: Order,
 ...
 * ```
 *
 * The only difference from join() is `JoinType.RIGHT` instead of `JoinType.INNER`.
 */
class RightJoinMethodGenerator(
    ...
    override fun generate(): String = buildString {
        ...
        // RIGHT JOIN: Compute target JP by appending RIGHT to source pattern
        val targetPattern = if (fromCombination.joinedTables.isEmpty()) {
            "RIGHT"
        } else {
            fromCombination.joinPattern + "_RIGHT"
        }
        ...
        appendLine("/**")
        appendLine(" * RIGHT OUTER JOIN ${joiningTable.capitalizedName} table.")
        appendLine(" */")
        appendLine("fun <$sourceAllParams>")
        appendLine("${fromCombination.builderClassName}<$sourceSelParams, AC, SourceJP>.rightJoin(")
