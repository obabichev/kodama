...
/**
 * Generates a GroupByContext class for GROUP BY clause operations.
 *
 * GroupByContext provides typed access to all tables in a query for building GROUP BY clauses.
 *
 * Example output for Person + Order combination:
 * ```
 * class GroupByContext_Person_Order(
 ...
 * //         ^^^^^ GroupByContext provides this
 * ```
 */
class GroupByContextGenerator(
    private val combination: QueryCombinationInfo,
    private val schemaPackage: String
) : CodeGenerator {

    override fun generate(): String = buildString {
        appendLine("class ${combination.groupByContextClassName}(")
        appendLine("    private val state: com.obabichev.kodama.query.QueryState")
        appendLine(") {")

        // Generate property for each table/subquery (using GroupByAccessor for GROUP BY context)
        combination.tables.forEach { table ->
            appendLine("    val ${table.camelCaseName}: ${table.capitalizedName}GroupByAccessor")

            // Different handling for subqueries vs regular tables
            if (table.isSubquery) {
                appendLine("        get() = ${table.capitalizedName}GroupByAccessor(TableAccessor(SubqueryRegistry.getOrCreate<${table.capitalizedName}>(), state.relations))")
            } else {
                appendLine("        get() = ${table.capitalizedName}GroupByAccessor(TableAccessor($schemaPackage.${table.capitalizedName}, state.relations))")
            }
            appendLine()
        }
