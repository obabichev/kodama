...
@Serializable
data class SerializableColumnMetadata(
...
@Serializable
data class SerializableTableMetadata(
    val name: String,
    val packageName: String,
    val qualifiedName: String,
    val columns: List<SerializableColumnMetadata>
)

@Serializable
data class RuntimeMetadataRoot(
    val tables: List<SerializableTableMetadata>
)
...
abstract class GenerateTableMetadataTask : DefaultTask() {
    ...
    @get:OutputFile
    abstract val runtimeMetadataJsonFile: RegularFileProperty
    ...
    fun generate() {
        ...
        // Write runtime metadata JSON for KSP processor
        writeRuntimeMetadataJson(tables)
    ...
    private fun writeRuntimeMetadataJson(tables: List<TableMetadata>) {
        val serializableTables = tables.map { table ->
        ...
        val metadata = RuntimeMetadataRoot(tables = serializableTables)
        val json = Json { prettyPrint = true }
        val jsonString = json.encodeToString(metadata)

        runtimeMetadataJsonFile.get().asFile.apply {
            parentFile.mkdirs()
            writeText(jsonString)
        }

        logger.lifecycle("Kodama: Wrote runtime metadata JSON with ${tables.size} tables")
    }
    ...
    private fun generateTableMetadataCode(
        ...
        // Import all unique packages that contain tables
        val tablePackages = tables.map { it.packageName }.distinct().sorted()
        ...
        tables.forEach { table ->
            appendLine("fun ${table.packageName}.${table.name}.all() = ${table.name}AllMarker(this)")
        }
        appendLine()
        ...
        tables.forEach { table ->
            ...
            table.columns.forEach { column ->
                ...
                appendLine("        get() = readColumn(${table.packageName}.${table.name}.${column.propertyName}) as ${column.kotlinType}")
                appendLine()
            }
            ...
            table.columns.forEach { column ->
                ...
                appendLine("        get() = readColumn(${table.packageName}.${table.name}.${column.propertyName}) as $nullableType")
                appendLine()
            }
        ...
        tables.forEach { table ->
            ...
            appendLine(" */")
            appendLine("fun ${table.packageName}.${table.name}.insert(")
        ...
        tables.forEach { table ->
            ...
            table.columns.forEach { column ->
                appendLine("    val ${column.propertyName} get() = OrderByColumn(${table.packageName}.${table.name}.${column.propertyName})")
            }
        ...
        tables.forEach { table ->
            ...
            table.columns.forEach { column ->
                appendLine("    val ${column.propertyName} get() = ${table.packageName}.${table.name}.${column.propertyName}")
            }
        ...
        tables.forEach { table ->
            ...
            appendLine(" */")
            appendLine("fun from(table: ${table.packageName}.${table.name}): $genPkg.QueryBuilder_1<$genPkg.${table.name}Marker, $genPkg.TableNotSelected, $genPkg.NoSelections> {")
